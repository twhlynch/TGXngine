...
def analyze_comment_text(comment_body):
    ...
    # FIX: Skip ASCII lines / decorative dividers (e.g., // ======= or // ----------)
    # Checks if the entire comment body is made of just 1 unique repeating symbol
...
def scan_for_commented_code(target_dirs):
                ...
                for line_no, line in enumerate(lines, 1):
                    ...
                    if in_block_comment:
                        if '*/' in line_str:
                            ...
                            if any(analyze_comment_text(l) for l in full_block_text.splitlines()):
                                print(f"{RED}Commented code block found in: {file_path}:{block_start_line}-{line_no}{RESET}")
                                print(f"   Context:\n{full_block_text}\n")
                                total_violations += 1
                                
                            in_block_comment = False
                            block_comment_accumulator = []
                            line_str = parts[1].strip()  # Keep scanning remainder of line
                        else:
                    ...
                    if '/*' in line_str:
                        if '*/' in line_str:
                            ...
                            if inline_match and analyze_comment_text(inline_match.group(1)):
                                print(f"{RED}Commented code found in: {file_path}:{line_no}{RESET}")
                    ...
                    if '//' in sanitized_line:
                        ...
                        if analyze_comment_text(comment_part):
                            print(f"{RED}Commented code line found in: {file_path}:{line_no}{RESET}")
                            print(f"   Line: '{line_str}'\n")
                            total_violations += 1
    ...
    if total_violations == 0:
        ...
        print(f"{RED}Scan complete. Found {total_violations} instances of commented code.{RESET}")
    print("-" * 60)

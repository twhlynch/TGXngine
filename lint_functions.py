...
FUNCTION_REGEX = re.compile(r'\b(?:([a-zA-Z_][a-zA-Z0-9_]*::))?([a-zA-Z_][a-zA-Z0-9_]*)\b\s*\(([^)]*)\)')
...
def is_valid_parameter_list(param_text):
    ...
    if re.match(r'^\s*-?\d+(?:\.\d+)?(?:f)?\s*$', param_text) or re.match(r'^\s*-?\d+\s*,\s*-?\d+\s*$', param_text):
        return False

    return True


def check_naming_violation(prefix, class_scope, func_name, param_text, active_assignments):
    """
    ...
    # Fix 1: If multiple statements share a single line, isolate the prefix of the current statement
    if prefix and ';' in prefix:
        prefix = prefix.split(';')[-1]

    # 1. Skip member access actions (e.g., window.Draw, pointer->Draw)
    if prefix.endswith('.') or prefix.endswith('->'):
        return False

    # 2. Skip constructor initializer list lines
    if prefix.endswith(',') or (':' in prefix and not prefix.endswith('::')):
        return False

    # 3. Skip class method declarations/definitions and explicit type instantiations
    if prefix:
        words = prefix.split()
        if words:
            last_word = words[-1].strip('&').strip('*')
            
            # Fix 2: Only skip if the last token is a core control-flow keyword (like if, while, return),
            # NOT when it's a valid return type modifier or primitive type name like void/bool/float.
    ...
    # 4. If an explicit scope was passed (e.g. Joystick::getAxisPosition) but there is no 
    ...
    if class_scope and class_scope.endswith('::'):
        if not prefix or not any(w in prefix for w in {'void', 'int', 'float', 'bool', 'auto', 'virtual', 'inline'}):
    ...
    # 5. Filter out object instantiations with literal parameters
    if not is_valid_parameter_list(param_text):
        return False

    # 6. Rule validation: Must be PascalCase
...
def scan_cpp_files(target_dirs):
    ...
    for target in target_dirs:
        ...
        if not target_path.exists():
            print(f"{YELLOW}Warning: Target directory not found: {target_path.absolute()}{RESET}")
            continue
        ...
        for file_path in target_path.rglob('*'):
            ...
            try:
                ...
                # Pre-scan step: Extract all identifiers that appear on the left side of an assignment (=)
                ...
                for line_no, line in enumerate(raw_text.splitlines(), 1):
                    ...
                    for match in FUNCTION_REGEX.finditer(line_clean):
                        ...
                        name_start_idx = match.start(1) if match.group(1) else match.start(2)
                        calculated_prefix = line_clean[:name_start_idx].rstrip()

                        if check_naming_violation(calculated_prefix, class_scope, func_name, param_text, active_assignments):
                            # Format output to include the class scope name if found
                            full_display_name = f"{class_scope}{func_name}"
                            print(f"{RED}Violation found in: {file_path}:{line_no}{RESET}")
    ...
    if total_violations == 0:
        print(f"{GREEN}Clean bill of health! All custom functions follow PascalCase.{RESET}")
    else:
        print(f"{RED}Scan complete. Found {total_violations} naming violation(s).{RESET}")
    print("-" * 60)
...
if __name__ == '__main__':
    ...
    if source_dir.exists() and source_dir.is_dir():
        ...
        print(f"{RED}Fatal Error: Could not find 'Source' folder at {source_dir}{RESET}")
        exit(1)
